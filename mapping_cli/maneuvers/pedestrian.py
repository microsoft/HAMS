...
def select_traj_points_by_time(tx, tz, segment_json, fps):
    ...
    frame_num_pedestrian_light_vid_start_abs = math.ceil(
        seg_vals["pedestrian"]["start"][0]
    )

    frame_num_red_light_off_abs = fps
    if frame_num_red_light_off_abs == -1:
        print("WARNING!!!!! pedestrian Light hook is not implemented")
        return list(zip(tx, tz))

    if (
        frame_num_pedestrian_light_vid_start_abs > frame_num_red_light_off_abs
    ):  # my video somehow cut afterwards
        return []

    end_frame_red_light_check = (
        frame_num_red_light_off_abs - frame_num_pedestrian_light_vid_start_abs
    )
...
def plot_traj(
    ...
    maneuver.report.add_report("pedestrianLight_obey_decision", obey_decision)
    maneuver.report.add_report("pedestrianLight_stop_decision", stop_decision)
    maneuver.report.add_report(
        "pedestrianLight_outcome",
        {
    ...
    plt.close()
    return obey_decision and stop_decision
...
class Pedestrian(Maneuver):
    def run(self):
        ...
        decision = plot_traj(
            ...
            save_image_name=os.path.join(self.out_folder, "pedestrian.png"),
        )
        ...
        return (
            (decision, stats),
            {  # TODO: Add pass/fail
                "trajectory": f"{os.path.join(self.out_folder, 'pedestrian.png')}"
            },
        )
