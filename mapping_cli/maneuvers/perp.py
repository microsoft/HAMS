...
class PERP(Maneuver):
    def run(self) -> None:
        ...
        if traj is False:
            raise Exception("Bad trajectory generated at perp. No results")

        _, tx, ty, tz, camera_matrices = traj
        if len(traj) < 30:
            if len(tx) > 99:
                tx, ty, tz = smoothen_trajectory(tx, ty, tz, 199, 299, 1)
            else:
                try:
                    tx, ty, tz = smoothen_trajectory(
                        tx, ty, tz, int(99 / 3), int(199 // 3), 2
                    )
                except:
                    window = len(tx) - 3 if len(tx) % 2 == 0 else len(tx) - 4
                    tx, ty, tz = smoothen_trajectory(tx, ty, tz, window, window, 2)
        else:
            raise Exception("Insufficient trajectory - check segmentation output")
        ...
        fig, ax = plt.subplots()
        is_inside, max_iou, max_iou_idx = is_car_inside(
            tx,
            ty,
            tz,
            camera_matrices,
        ...
        box_patch = mpatches.Polygon(box, alpha=1, fill=False, edgecolor="red")
        ax.add_patch(box_patch)
        if max_iou_idx is not None:
            print("Adding car patch")
            ...
            car_pts = get_car_box(
                ...
                self.config["car_dims"],
                self.config["is_vertical"],
            )
            car_patch = mpatches.Polygon(
                car_pts, alpha=1, fill=False, edgecolor="green", label="vehicle"
            )
        ...
        return (
            (is_inside and std_path_decision, stats),
            {
                ...
                "perp_standard_path": f"{os.path.join(self.out_folder, 'perp_trajectory.png')}",
                "perp_trajectory_path": f"{os.path.join(self.out_folder, 'standard_path_debug_perp.png')}",
            },
        )
...
def get_direction_stats(
    ...
    debug_directions_visualize(tx, ty, tz, directions, markers, out_folder)
...
def debug_directions_visualize(tx, ty, tz, directions, markers, out_folder):
    """ """
    ...
    plot_limits()
    plot_legend()
    plt.savefig(os.path.join(out_folder, "perp_trajectory.png"))
    plt.clf()
...
def plot_car(ax, max_iou_idx, tx, ty, tz, camera_matrices, car_dims, is_vertical):
    if max_iou_idx is not None:
        print("Adding car patch")
        position = max_iou_idx
        car_pts = get_car_box(
...
def check_standard_path(
    ...
    print("Manu order: ", manu_order, " substrings: ", substrings)
